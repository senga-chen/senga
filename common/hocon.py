...
from common._config import Config
from common.errors import HoconParserException, HoconTokenizerException
...
class HoconObject(HoconElement, dict):
    ...
    def merge(self, obj):
        for k, v in obj.iteritems():
            if k in self:
                thisItem = self[k]
                if thisItem.isObject() and v.value.isObject():
                    thisItem.getObject.merge(v.value.getObject())
            else:
                self[k] = v.value
